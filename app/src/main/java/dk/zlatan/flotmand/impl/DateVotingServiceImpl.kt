...
class DateVotingServiceImpl
    @Inject
    constructor() : DateVotingService {
        override fun observeVotingByIdFlow(votingId: String): Flow<DateVotingItem?> =
            callbackFlow {
                val docRef =
                    Firebase.firestore
                        ...
                        Firebase.firestore
                            ...
                            .addSnapshotListener { snapshot, error ->
                                ...
                                votings.forEachIndexed { index, voting ->
                                    Log.d(
                                        TAG,
                                        "  [$index] votingId=${voting.votingId}, creatorId=${voting.creatorId}, dateOptions=${voting.dateOptions.size}, status=${voting.status}",
                                    )
                                }
        ...
        override suspend fun deleteVoteOption(
            ...
            try {
                val voting = getDateVoting(votingId) ?: return
                val updatedOptions =
                    voting.dateOptions.filterNot { it.localDate == voteOption.localDate }
        ...
        override suspend fun addDateOption(
            votingId: String,
            date: LocalDate,
            userId: String,
        ) {
            try {
                ...
                val newOption =
                    DateOption.fromLocalDate(date).copy(
                ...
                Log.d(
                    TAG,
                    "Added date option $date to voting $votingId with user $userId automatically voted",
                )
            } catch (e: Exception) {
